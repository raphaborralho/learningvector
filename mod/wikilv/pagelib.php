 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class page_wikilv {

    /**
     * @var object Current subwikilv
     */
    protected $subwikilv;
    ...
    protected $uid;
    /**
     * @var array The tabs set used in wikilv module
     */
     ...
     *
     * @param $wikilv. Current wikilv
     * @param $subwikilv. Current subwikilv.
    ...
    function print_header() {
        ...
        if (isset($SESSION->wikilvpreviousurl) && is_array($SESSION->wikilvpreviousurl)) {
        ...
        if (!empty($this->page) && !empty($this->tabs)) {
            echo $this->wikilvoutput->tabs($this->page, $this->tabs, $this->tabs_options);
        }
    }
    ...
    protected function setup_tabs($options = array()) {
        ...
        if (!has_capability('mod/wikilv:editpage', $PAGE->context)){
    ...
    function print_content() {
        throw new coding_exception('Page wikilv class does not implement method print_content()');
    }
    ...
    protected function set_url() {
        throw new coding_exception('Page wikilv class does not implement method set_url()');
    }
    ...
    protected function create_navbar() {
        ...
        $PAGE->navbar->add(format_string($this->title), $CFG->wwwroot . '/mod/wikilv/view.php?pageid=' . $this->page->id);
    }
    ...
    protected function process_session_url() {
        ...
        $url = $SESSION->wikilvpreviousurl;
        switch ($url['page']) {
        case 'edit':
            wikilv_delete_locks($url['params']['pageid'], $USER->id, $url['params']['section'], false);
            break;
        }
    }
    ...
    protected function set_session_url() {
        global $SESSION;
        unset($SESSION->wikilvpreviousurl);
    }
...
class page_wikilv_view extends page_wikilv {
    ...
    function print_header() {
        ...
        $this->wikilvoutput->wikilv_print_subwikilv_selector($PAGE->activityrecord, $this->subwikilv, $this->page, 'view');

        if (!empty($this->page)) {
            echo $this->wikilvoutput->prettyview_link($this->page);
        }

        //echo $this->wikilvoutput->page_index();
    ...
    function print_content() {
        ...
        if (wikilv_user_can_view($this->subwikilv)) {

            if (!empty($this->page)) {
                wikilv_print_page_content($this->page, $this->modcontext, $this->subwikilv->id);
                $wikilv = $PAGE->activityrecord;
            } else {
                print_string('nocontent', 'wikilv');
                // TODO: fix this part
                $swid = 0;
                if (!empty($this->subwikilv)) {
                    $swid = $this->subwikilv->id;
                }
            }
        } else {
            echo get_string('cannotviewpage', 'wikilv');
        }
    }
    ...
    protected function create_navbar() {
        ...
        $PAGE->navbar->add(get_string('view', 'wikilv'));
    }
}
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class page_wikilv_edit extends page_wikilv {
    ...
    function print_header() {
        global $OUTPUT, $PAGE;
        $PAGE->requires->data_for_js('wikilv', array('renew_lock_timeout' => LOCK_TIMEOUT - 5, 'pageid' => $this->page->id, 'section' => $this->section));
        $PAGE->requires->js_init_call('M.block_lvs.desabilitar_copiar_colar'); // @lvs desabilitando copiar/colar

        parent::print_header();

        $this->print_pagetitle();

        print '<noscript>' . $OUTPUT->box(get_string('javascriptdisabledlocks', 'wikilv'), 'errorbox') . '</noscript>';
    }

    function print_content() {
        global $PAGE;

        if (wikilv_user_can_edit($this->subwikilv)) {
            $this->print_edit();
        } else {
    ...
    protected function set_url() {
        ...
        $PAGE->set_url($CFG->wwwroot . '/mod/wikilv/edit.php', $params);
    }

    protected function set_session_url() {
        global $SESSION;

        $SESSION->wikilvpreviousurl = array('page' => 'edit', 'params' => array('pageid' => $this->page->id, 'section' => $this->section));
    }
    ...
    protected function create_navbar() {
        ...
        $PAGE->navbar->add(get_string('edit', 'wikilv'));
    }

    protected function check_locks() {
        global $OUTPUT, $USER, $CFG;

        if (!wikilv_set_lock($this->page->id, $USER->id, $this->section, true)) {
            print $OUTPUT->box(get_string('pageislocked', 'wikilv'), 'generalbox boxwidthnormal boxaligncenter');
            ...
            if ($this->overridelock) {
                ...
                $form = '<form method="post" action="' . $CFG->wwwroot . '/mod/wikilv/overridelocks.php?' . $params . '">';
                $form .= '<input type="hidden" name="sesskey" value="' . sesskey() . '" />';
                $form .= '<input type="submit" value="' . get_string('overridelocks', 'wikilv') . '" />';
                $form .= '</form>';
    ...
    protected function print_edit($content = null) {
        ...
        wikilv_delete_old_locks();

        $version = wikilv_get_current_version($this->page->id);
        ...
        if ($this->versionnumber >= 0) {
            if ($version->version != $this->versionnumber) {
                print $OUTPUT->box(get_string('wrongversionlock', 'wikilv'), 'errorbox');
                $versionnumber = $this->versionnumber;
            }
        }

        $url = $CFG->wwwroot . '/mod/wikilv/edit.php?pageid=' . $this->page->id;
        if (!empty($this->section)) {
            $url .= "&section=" . urlencode($this->section);
        }

        $params = array(
            'attachmentoptions' => page_wikilv_edit::$attachmentoptions,
            'format' => $version->contentformat,
        ...
        case 'html':
            ...
            page_wikilv_edit::$attachmentoptions = array_merge(array('context' => $this->modcontext), page_wikilv_edit::$attachmentoptions);
            $data = file_prepare_standard_editor($data, 'newcontent', page_wikilv_edit::$attachmentoptions, $this->modcontext, 'mod_wikilv', 'attachments', $this->subwikilv->id);
            break;
        default:
            break;
        }
        ...
        if ($version->contentformat != 'html') {
            $params['fileitemid'] = $this->subwikilv->id;
            $params['component']  = 'mod_wikilv';
...
/**
 * Class that models the behavior of wikilv's view comments page
 *
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class page_wikilv_comments extends page_wikilv {
    ...
    function print_content() {
        ...
        $page = $this->page;
        $subwikilv = $this->subwikilv;
        $wikilv = $PAGE->activityrecord;
        list($context, $course, $cm) = get_context_info_array($this->modcontext->id);

        require_capability('mod/wikilv:viewcomment', $this->modcontext, NULL, true, 'noviewcommentpermission', 'wikilv');

        $comments = wikilv_get_comments($this->modcontext->id, $page->id);

        if (has_capability('mod/wikilv:editcomment', $this->modcontext)) {
            echo '<div class="midpad"><a href="' . $CFG->wwwroot . '/mod/wikilv/editcomments.php?action=add&amp;pageid=' . $page->id . '">' . get_string('addcomment', 'wikilv') . '</a></div>';
        }

        $options = array('swid' => $this->page->subwikilvid, 'pageid' => $page->id);
        $version = wikilv_get_current_version($this->page->id);
        $format = $version->contentformat;
        ...
        foreach ($comments as $comment) {
            ...
            $user = wikilv_get_user_info($comment->userid);
            ...
            if ($format != 'html') {
                if ($format == 'creole') {
                    $parsedcontent = wikilv_parse_content('creole', $comment->content, $options);
                } else if ($format == 'nwikilv') {
                    $parsedcontent = wikilv_parse_content('nwikilv', $comment->content, $options);
                }
            ...
            //$cell4->text .= $gerenciadorNotas->formAvaliacaoAjax($item);
            // lvs fim
    ...
    function set_url() {
        global $PAGE, $CFG;
        $PAGE->set_url($CFG->wwwroot . '/mod/wikilv/comments.php', array('pageid' => $this->page->id));
    }
    ...
    protected function create_navbar() {
        ...
        parent::create_navbar();
        $PAGE->navbar->add(get_string('comments', 'wikilv'));
    }

}

/**
 * Class that models the behavior of wikilv's edit comment
 *
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class page_wikilv_editcomment extends page_wikilv {
    private $comment;
    private $action;
    private $form;
    private $format;
    ...
    function print_content() {
        ...
        require_capability('mod/wikilv:editcomment', $this->modcontext, NULL, true, 'noeditcommentpermission', 'wikilv');
    ...
    function set_action($action, $comment) {
        ...
        if ($this->format == 'html') {
            $destination = $CFG->wwwroot . '/mod/wikilv/instancecomments.php?pageid=' . $this->page->id;
            $this->form = new mod_wikilv_comments_form($destination);
        }
    }
    ...
    protected function create_navbar() {
        ...
        $PAGE->navbar->add(get_string('comments', 'wikilv'), $CFG->wwwroot . '/mod/wikilv/comments.php?pageid=' . $this->page->id);

        if ($this->action == 'add') {
            $PAGE->navbar->add(get_string('insertcomment', 'wikilv'));
        } else {
            $PAGE->navbar->add(get_string('editcomment', 'wikilv'));
        }
    }
    ...
    private function add_comment_form() {
        global $CFG;
        require_once($CFG->dirroot . '/mod/wikilv/editors/wikilv_editor.php');
        ...
        if ($this->format == 'html') {
            ...
            wikilv_print_editor_wikilv($this->page->id, null, $this->format, -1, null, false, null, 'addcomments');
        }
    }
    ...
    private function edit_comment_form($com) {
        ...
        if ($this->format == 'html') {
            ...
            wikilv_print_editor_wikilv($this->page->id, $com->content, $this->format, -1, null, false, array(), 'editcomments', $com->id);
        }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class page_wikilv_search extends page_wikilv {
    ...
    function set_search_string($search, $searchcontent) {
        $swid = $this->subwikilv->id;
        if ($searchcontent) {
            $this->search_result = wikilv_search_all($swid, $search);
        } else {
            $this->search_result = wikilv_search_title($swid, $search);
        }
    ...
    function print_content() {
        ...
        require_capability('mod/wikilv:viewpage', $this->modcontext, NULL, true, 'noviewpagepermission', 'wikilv');

        echo $this->wikilvoutput->search_result($this->search_result, $this->subwikilv);
    }
}

/**
 *
 * Class that models the behavior of wikilv's
 * create page
 *
 */
class page_wikilv_create extends page_wikilv {
    ...
    function set_url() {
        ...
        if ($this->action == 'new') {
            $params['action'] = 'new';
            $params['wid'] = $this->wid;
            if ($this->title != get_string('newpage', 'wikilv')) {
                $params['title'] = $this->title;
            }
        } else {
            $params['action'] = 'create';
        }
        $PAGE->set_url(new moodle_url('/mod/wikilv/create.php', $params));
    }
    ...
    function set_action($action) {
        ...
        $url = new moodle_url('/mod/wikilv/create.php', array('action' => 'create', 'wid' => $PAGE->activityrecord->id, 'group' => $this->gid, 'uid' => $this->uid));
        ...
        $this->mform = new mod_wikilv_create_form($url->out(false), $options);
    }
    ...
    function print_content($pagetitle = '') {
        ...
        require_capability('mod/wikilv:createpage', $this->modcontext, NULL, true, 'nocreatepermission', 'wikilv');
        $data = new stdClass();
        if (!empty($pagetitle)) {
    ...
    function create_page($pagetitle) {
        ...
        if (empty($this->subwikilv)) {
            // If subwikilv is not set then try find one and set else create one.
            if (!$this->subwikilv = wikilv_get_subwikilv_by_group($this->wid, $groupid, $this->uid)) {
                $swid = wikilv_add_subwikilv($PAGE->activityrecord->id, $groupid, $this->uid);
                $this->subwikilv = wikilv_get_subwikilv($swid);
            }
        }
        if ($data) {
            $this->set_title($data->pagetitle);
            $id = wikilv_create_page($this->subwikilv->id, $data->pagetitle, $data->pageformat, $USER->id);
        } else {
            $this->set_title($pagetitle);
            $id = wikilv_create_page($this->subwikilv->id, $pagetitle, $PAGE->activityrecord->defaultformat, $USER->id);
        }
        $this->page = $id;
        return $id;
    }
...
class page_wikilv_preview extends page_wikilv_edit {
    ...
    function __construct($wikilv, $subwikilv, $cm) {
        ...
        parent::__construct($wikilv, $subwikilv, $cm);
        $buttons = $OUTPUT->update_module_button($cm->id, 'wikilv');
    ...
    function print_content() {
        ...
        require_capability('mod/wikilv:editpage', $this->modcontext, NULL, true, 'noeditpermission', 'wikilv');
    ...
    protected function print_preview() {
        ...
        $options = array('swid' => $this->page->subwikilvid, 'pageid' => $this->page->id, 'pretty_print' => true);
 ...
 *
 */
class page_wikilv_diff extends page_wikilv {
    ...
    function set_url() {
        ...
        $PAGE->set_url($CFG->wwwroot . '/mod/wikilv/diff.php', array('pageid' => $this->page->id, 'comparewith' => $this->comparewith, 'compare' => $this->compare));
    }
    ...
    protected function create_navbar() {
        ...
        parent::create_navbar();
        $PAGE->navbar->add(get_string('history', 'wikilv'), $CFG->wwwroot . '/mod/wikilv/history.php?pageid=' . $this->page->id);
        $PAGE->navbar->add(get_string('diff', 'wikilv'));
    }
    ...
    private function print_diff_content() {
        ...
        $pageid = $this->page->id;
        $total = wikilv_count_wikilv_page_versions($pageid) - 1;

        $oldversion = wikilv_get_wikilv_page_version($pageid, $this->compare);

        $newversion = wikilv_get_wikilv_page_version($pageid, $this->comparewith);

        if ($oldversion && $newversion) {

            $oldtext = format_text(file_rewrite_pluginfile_urls($oldversion->content, 'pluginfile.php', $this->modcontext->id, 'mod_wikilv', 'attachments', $this->subwikilv->id));
            $newtext = format_text(file_rewrite_pluginfile_urls($newversion->content, 'pluginfile.php', $this->modcontext->id, 'mod_wikilv', 'attachments', $this->subwikilv->id));
            list($diff1, $diff2) = ouwikilv_diff_html($oldtext, $newtext);
            $oldversion->diff = $diff1;
            $oldversion->user = wikilv_get_user_info($oldversion->userid);
            $newversion->diff = $diff2;
            $newversion->user = wikilv_get_user_info($newversion->userid);

            echo $this->wikilvoutput->diff($pageid, $oldversion, $newversion, array('total' => $total));
        } else {
            print_error('versionerror', 'wikilv');
        }
    }
}
...
/**
 *
 * Class that models the behavior of wikilv's history page
 *
 */
class page_wikilv_history extends page_wikilv {
    /**
    ...
    function __construct($wikilv, $subwikilv, $cm) {
        global $PAGE;
        parent::__construct($wikilv, $subwikilv, $cm);
        $PAGE->requires->js_init_call('M.mod_wikilv.history', null, true);
    }
    ...
    function set_url() {
        global $PAGE, $CFG;
        $PAGE->set_url($CFG->wwwroot . '/mod/wikilv/history.php', array('pageid' => $this->page->id));
    }
    ...
    protected function create_navbar() {
        ...
        parent::create_navbar();
        $PAGE->navbar->add(get_string('history', 'wikilv'));
    }

    /**
     * Prints the history for a given wikilv page
     *
     * @global object $CFG
     * @global object $OUTPUT
     * @global object $PAGE
    ...
    private function print_history_content() {
        ...
        $vcount = wikilv_count_wikilv_page_versions($pageid) - 1;
        if ($this->allversion) {
            $versions = wikilv_get_wikilv_page_versions($pageid, 0, $vcount);
        } else {
            $versions = wikilv_get_wikilv_page_versions($pageid, $offset, $this->rowsperpage);
        }
        ...
        $version0page = wikilv_get_wikilv_page_version($this->page->id, 0);
        $creator = wikilv_get_user_info($version0page->userid);
        $a = new StdClass;
        ...
        if ($vcount > 0) {
            ...
            if (count($versions) == 1) {
                ...
                $username = wikilv_get_user_info($row->userid);
                ...
                $gerenciadorNotas = NotasLvFactory::criarGerenciador('moodle2');
                $gerenciadorNotas->setModulo( new Wikilv($wikilv->id) );
                $item = new Item('wikilv', 'version', $row);
                
                if ($gerenciadorNotas->podeVerNota($item)) {
                	$carinhaslvs = new Carinhas();
                	$nota = $gerenciadorNotas->getAvaliacao($item)->getNota();
                	$carinhalv = ($nota !== null) ? $carinhaslvs->recuperarCarinhaHtml($nota) : '';
                } else {
                	$carinhalv = '';
                }
                // lvs fim
                ...
                foreach ($versions as $version) {
                    $user = wikilv_get_user_info($version->userid);
                    $picture = $OUTPUT->user_picture($user, array('popup' => true));
                    $date = userdate($version->timecreated, get_string('strftimedate'));
                    $rowclass[] = 'wikilv_histnewdate';
                    $time = userdate($version->timecreated, get_string('strftimetime', 'langconfig'));
                    $versionid = wikilv_get_version($version->id);
                    if ($versionid) {
                        $url = new moodle_url('/mod/wikilv/viewversion.php', array('pageid' => $pageid, 'versionid' => $versionid->id));
                ...
                // Print the form.
                echo html_writer::start_tag('form', array('action'=>new moodle_url('/mod/wikilv/diff.php'), 'method'=>'get', 'id'=>'diff'));
                echo html_writer::tag('div', html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'pageid', 'value'=>$pageid)));
                echo html_writer::table($table);
                echo html_writer::start_tag('div', array('class'=>'mdl-align'));
                echo html_writer::empty_tag('input', array('type'=>'submit', 'class'=>'wikilv_form-button', 'value'=>get_string('comparesel', 'wikilv')));
                echo html_writer::end_tag('div');
                echo html_writer::end_tag('form');
            }
        } else {
            print_string('nohistory', 'wikilv');
        }
        if (!$this->allversion) {
            //$pagingbar = moodle_paging_bar::make($vcount, $this->paging, $this->rowsperpage, $CFG->wwwroot.'/mod/wikilv/history.php?pageid='.$pageid.'&amp;');
            // $pagingbar->pagevar = $pagevar;
            echo $OUTPUT->paging_bar($vcount, $this->paging, $this->rowsperpage, $CFG->wwwroot . '/mod/wikilv/history.php?pageid=' . $pageid . '&amp;');
            //print_paging_bar($vcount, $paging, $rowsperpage,$CFG->wwwroot.'/mod/wikilv/history.php?pageid='.$pageid.'&amp;','paging');
            } else {
            $link = new moodle_url('/mod/wikilv/history.php', array('pageid' => $pageid));
            $OUTPUT->container(html_writer::link($link->out(false), get_string('viewperpage', 'wikilv', $this->rowsperpage)), 'mdl-align');
        }
        if ($vcount > $this->rowsperpage && !$this->allversion) {
            $link = new moodle_url('/mod/wikilv/history.php', array('pageid' => $pageid, 'allversion' => 1));
            $OUTPUT->container(html_writer::link($link->out(false), get_string('viewallhistory', 'wikilv')), 'mdl-align');
        }
    }
...
/**
 * Class that models the behavior of wikilv's map page
 *
 */
class page_wikilv_map extends page_wikilv {

    /**
     * @var int wikilv view option
     */
    private $view;
    ...
    function print_content() {
        ...
        if ($this->view > 0) {
            //echo '<div><a href="' . $CFG->wwwroot . '/mod/wikilv/map.php?pageid=' . $this->page->id . '">' . get_string('backtomapmenu', 'wikilv') . '</a></div>';
        }
    ...
    function set_url() {
        global $PAGE, $CFG;
        $PAGE->set_url($CFG->wwwroot . '/mod/wikilv/map.php', array('pageid' => $this->page->id));
    }
    ...
    protected function create_navbar() {
        ...
        parent::create_navbar();
        $PAGE->navbar->add(get_string('map', 'wikilv'));
    }
    ...
    private function print_contributions_content() {
        ...
        if ($page->timerendered + WIKILV_REFRESH_CACHE_TIME < time()) {
            $fresh = wikilv_refresh_cachedcontent($page);
            $page = $fresh['page'];
        }
        ...
        $table = new html_table();
        $table->head = array(get_string('contributions', 'wikilv') . $OUTPUT->help_icon('contributions', 'wikilv'));
        $table->attributes['class'] = 'wikilv_editor generalbox';
        $table->data = array();
        ...
        if ($contribs = wikilv_get_contributions($swid, $USER->id)) {
            foreach ($contribs as $contrib) {
                if (!array_key_exists($contrib->pageid, $pages)) {
                    $page = wikilv_get_page($contrib->pageid);
                ...
                if (!array_key_exists($page->id, $lastversions)) {
                    $version = wikilv_get_last_version($page->id);
                ...
                $link = wikilv_parser_link($page->title, array('swid' => $swid));
                $class = ($link['new']) ? 'class="wikilv_newentry"' : '';
            ...
            $table->data[] = array(get_string('nocontribs', 'wikilv'));
        }
        echo html_writer::table($table);
    }
    ...
    private function print_navigation_content() {
        ...
        $tolinks = wikilv_get_linked_to_pages($page->id);
        $fromlinks = wikilv_get_linked_from_pages($page->id);

        $table = new html_table();
        $table->attributes['class'] = 'wikilv_navigation_from';
        $table->head = array(get_string('navigationfrom', 'wikilv') . $OUTPUT->help_icon('navigationfrom', 'wikilv') . ':');
        $table->data = array();
        $table->rowclasses = array();
        foreach ($fromlinks as $link) {
            $lpage = wikilv_get_page($link->frompageid);
            $link = new moodle_url('/mod/wikilv/view.php', array('pageid' => $lpage->id));
        ...
        $table = new html_table();
        $table->attributes['class'] = 'wikilv_navigation_to';
        $table->head = array(get_string('navigationto', 'wikilv') . $OUTPUT->help_icon('navigationto', 'wikilv') . ':');
        $table->data = array();
        $table->rowclasses = array();
        foreach ($tolinks as $link) {
            if ($link->tomissingpage) {
                $viewlink = new moodle_url('/mod/wikilv/create.php', array('swid' => $page->subwikilvid, 'title' => $link->tomissingpage, 'action' => 'new'));
                $table->data[] = array(html_writer::link($viewlink->out(false), format_string($link->tomissingpage), array('class' => 'wikilv_newentry')));
            } else {
                $lpage = wikilv_get_page($link->topageid);
                $viewlink = new moodle_url('/mod/wikilv/view.php', array('pageid' => $lpage->id));
                $table->data[] = array(html_writer::link($viewlink->out(false), format_string($lpage->title)));
            }
            $table->rowclasses[] = 'mdl-align';
        }
        $table_right = html_writer::table($table);
        echo $OUTPUT->container($table_left . $table_right, 'wikilv_navigation_container');
    }
    ...
    private function print_index_content() {
        ...
        $keys = array();
        $tree = array();
        $tree = wikilv_build_tree($page, $node, $keys);

        $table = new html_table();
        $table->head = array(get_string('pageindex', 'wikilv') . $OUTPUT->help_icon('pageindex', 'wikilv'));
        $table->attributes['class'] = 'wikilv_editor generalbox';
    ...
    private function print_page_list_content() {
        ...
        $pages = wikilv_get_page_list($this->subwikilv->id);

        $stdaux = new stdClass();
        $strspecial = get_string('special', 'wikilv');
        ...
        $table = new html_table();
        $table->head = array(get_string('pagelist', 'wikilv') . $OUTPUT->help_icon('pagelist', 'wikilv'));
        $table->attributes['class'] = 'wikilv_editor generalbox';
    ...
    private function print_orphaned_content() {
        ...
        $table = new html_table();
        $table->head = array(get_string('orphaned', 'wikilv') . $OUTPUT->help_icon('orphaned', 'wikilv'));
        $table->attributes['class'] = 'wikilv_editor generalbox';
        $table->data = array();
        $table->rowclasses = array();

        if ($orphanedpages = wikilv_get_orphaned_pages($swid)) {
            ...
            $table->data[] = array(get_string('noorphanedpages', 'wikilv'));
        }
    ...
    private function print_updated_content() {
        ...
        $table = new html_table();
        $table->head = array(get_string('updatedpages', 'wikilv') . $OUTPUT->help_icon('updatedpages', 'wikilv'));
        $table->attributes['class'] = 'wikilv_editor generalbox';
        $table->data = array();
        $table->rowclasses = array();

        if ($pages = wikilv_get_updated_pages_by_subwikilv($swid)) {
            $strdataux = '';
            foreach ($pages as $page) {
                $user = wikilv_get_user_info($page->userid);
            ...
            $table->data[] = array(get_string('noupdatedpages', 'wikilv'));
        }
    ...
    protected function render_navigation_node($items, $attrs = array(), $expansionlimit = null, $depth = 1) {
        ...
        foreach ($items as $item) {
            ...
            if ($item->icon instanceof renderable) {
                $icon = $this->wikilvoutput->render($item->icon);
...
/**
 * Class that models the behavior of wikilv's restore version page
 *
 */
class page_wikilv_restoreversion extends page_wikilv {
    private $version;
    ...
    function set_versionid($versionid) {
        $this->version = wikilv_get_version($versionid);
    }
    ...
    protected function create_navbar() {
        ...
        parent::create_navbar();
        $PAGE->navbar->add(get_string('restoreversion', 'wikilv'));
    }
...
/**
 * Class that models the behavior of wikilv's delete comment confirmation page
 *
 */
class page_wikilv_deletecomment extends page_wikilv {
    private $commentid;
    ...
    function set_url() {
        global $PAGE;
        $PAGE->set_url('/mod/wikilv/instancecomments.php', array('pageid' => $this->page->id, 'commentid' => $this->commentid));
    }
    ...
    protected function create_navbar() {
        ...
        parent::create_navbar();
        $PAGE->navbar->add(get_string('deletecommentcheck', 'wikilv'));
    }
    ...
    private function printconfirmdelete() {
        ...
        $strdeletecheck = get_string('deletecommentcheck', 'wikilv');
        $strdeletecheckfull = get_string('deletecommentcheckfull', 'wikilv');
 ...
 *
 */
class page_wikilv_save extends page_wikilv_edit {
    ...
    protected function print_save() {
        ...
        if ($data = $form->get_data()) {
            if ($this->format == 'html') {
                $data = file_postupdate_standard_editor($data, 'newcontent', page_wikilv_edit::$attachmentoptions, $this->modcontext, 'mod_wikilv', 'attachments', $this->subwikilv->id);
            }

            if (isset($this->section)) {
                $save = wikilv_save_section($this->page, $this->section, $data->newcontent, $USER->id);
            } else {
                $save = wikilv_save_page($this->page, $data->newcontent, $USER->id);
            }
        }
        ...
        if ($save && $data) {
            ...
            if (!empty($save['sections'])) {
                foreach ($save['sections'] as $s) {
                    $message .= '<p>' . get_string('repeatedsection', 'wikilv', $s) . '</p>';
                }
            }

            if ($this->versionnumber + 1 != $save['version']) {
                $message .= '<p>' . get_string('wrongversionsave', 'wikilv') . '</p>';
            }

            if (isset($errors) && !empty($errors)) {
                foreach ($errors as $e) {
                    $message .= "<p>" . get_string('filenotuploadederror', 'wikilv', $e->get_filename()) . "</p>";
                }
            }

            //deleting old locks
            wikilv_delete_locks($this->page->id, $USER->id, $this->section);
            $url = new moodle_url('/mod/wikilv/view.php', array('pageid' => $this->page->id, 'group' => $this->subwikilv->groupid));
            redirect($url);
        } else {
            print_error('savingerror', 'wikilv');
        }
    }
}

/**
 * Class that models the behavior of wikilv's view an old version of a page
 *
 */
class page_wikilv_viewversion extends page_wikilv {
    ...
    protected function create_navbar() {
        ...
        parent::create_navbar();
        $PAGE->navbar->add(get_string('history', 'wikilv'), $CFG->wwwroot . '/mod/wikilv/history.php?pageid=' . $this->page->id);
        $PAGE->navbar->add(get_string('versionnum', 'wikilv', $this->version->version));
    }
    ...
    private function print_version_view() {
        global $CFG, $OUTPUT, $PAGE;
        $pageversion = wikilv_get_version($this->version->id);
...
class page_wikilv_confirmrestore extends page_wikilv_save {
...
class page_wikilv_prettyview extends page_wikilv {
    ...
    function set_url() {
        ...
        $PAGE->set_url($CFG->wwwroot . '/mod/wikilv/prettyview.php', array('pageid' => $this->page->id));
    }
...
class page_wikilv_handlecomments extends page_wikilv {
    private $action;
    private $content;
    private $commentid;
    private $format;
    ...
    private function add_comment($content, $idcomment) {
        ...
        wikilv_add_comment($this->modcontext, $pageid, $content, $this->format);

        if (!$idcomment) {
            redirect($CFG->wwwroot . '/mod/wikilv/comments.php?pageid=' . $pageid, get_string('createcomment', 'wikilv'), 2);
        } else {
            $this->delete_comment($idcomment);
            redirect($CFG->wwwroot . '/mod/wikilv/comments.php?pageid=' . $pageid, get_string('editingcomment', 'wikilv'), 2);
        }
    }
    ...
    private function delete_comment($commentid) {
        ...
        wikilv_delete_comment($commentid, $this->modcontext, $pageid);
    }

}

class page_wikilv_lock extends page_wikilv_edit {
    ...
    protected function set_url() {
        ...
        $PAGE->set_url($CFG->wwwroot . '/mod/wikilv/lock.php', $params);
    }
    ...
    public function print_content() {
        ...
        wikilv_set_lock($this->page->id, $USER->id, $this->section);
    }
...
class page_wikilv_overridelocks extends page_wikilv_edit {
    function print_header() {
        $this->set_url();
    }
    ...
    function print_content() {
        ...
        require_capability('mod/wikilv:overridelock', $this->modcontext, NULL, true, 'nooverridelockpermission', 'wikilv');

        wikilv_delete_locks($this->page->id, null, $this->section, true, true);
        ...
        redirect($CFG->wwwroot . '/mod/wikilv/edit.php?' . $args, get_string('overridinglocks', 'wikilv'), 2);
    }
    ...
    function set_url() {
        ...
        $PAGE->set_url($CFG->wwwroot . '/mod/wikilv/overridelocks.php', $params);
    }
...
/**
 * This class will let user to delete wikilv pages and page versions
 *
 */
class page_wikilv_admin extends page_wikilv {
     ...
     *
     * @global object $PAGE
     * @param mixed $wikilv instance of wikilv
     * @param mixed $subwikilv instance of subwikilv
     * @param stdClass $cm course module
     */
    function __construct($wikilv, $subwikilv, $cm) {
        global $PAGE;
        parent::__construct($wikilv, $subwikilv, $cm);
        $PAGE->requires->js_init_call('M.mod_wikilv.deleteversion', null, true);
    }

    /**
     * Prints header for wikilv page
     */
    function print_header() {
        parent::print_header();
        $this->print_pagetitle();
    ...
    /**
     * This function will display administration view to users with managewikilv capability
     */
    function print_content() {
        //make sure anyone trying to access this page has managewikilv capabilities
        require_capability('mod/wikilv:managewikilv', $this->modcontext, NULL, true, 'noviewpagepermission', 'wikilv');

        //update wikilv cache if timedout
        $page = $this->page;
        if ($page->timerendered + WIKILV_REFRESH_CACHE_TIME < time()) {
            $fresh = wikilv_refresh_cachedcontent($page);
            $page = $fresh['page'];
        ...
        //dispaly admin menu
        echo $this->wikilvoutput->menu_admin($this->page->id, $this->view);
    ...
    function set_url() {
        global $PAGE, $CFG;
        $PAGE->set_url($CFG->wwwroot . '/mod/wikilv/admin.php', array('pageid' => $this->page->id));
    }
    ...
    protected function create_navbar() {
        ...
        parent::create_navbar();
        $PAGE->navbar->add(get_string('admin', 'wikilv'));
    }

    /**
     * Show wikilv page delete options
     *
    ...
    protected function print_delete_content($showorphan = true) {
        $contents = array();
        $table = new html_table();
        $table->head = array('', get_string('pagename','wikilv'));
        $table->attributes['class'] = 'generaltable mdl-align';
        $swid = $this->subwikilv->id;
        if ($showorphan) {
            if ($orphanedpages = wikilv_get_orphaned_pages($swid)) {
            ...
            if ($pages = wikilv_get_page_list($swid)) {
                $this->add_page_delete_options($pages, $swid, $table);
            } else {
                $table->data[] = array('', get_string('nopages', 'wikilv'));
            }
        }

        ///Print the form
        echo html_writer::start_tag('form', array(
                                                'action' => new moodle_url('/mod/wikilv/admin.php'),
                                                'method' => 'post'));
        ...
        if (!$showorphan) {
            echo html_writer::empty_tag('input', array(
                                                     'type'    => 'submit',
                                                     'class'   => 'wikilv_form-button',
                                                     'value'   => get_string('listorphan', 'wikilv'),
                                                     'sesskey' => sesskey()));
        } else {
            ...
            echo html_writer::empty_tag('input', array(
                                                     'type'    => 'submit',
                                                     'class'   => 'wikilv_form-button',
                                                     'value'   => get_string('listall', 'wikilv'),
                                                     'sesskey' => sesskey()));
        }
        echo html_writer::end_tag('div');
        echo html_writer::end_tag('form');
     ...
     *
     * @global object $OUTPUT
     * @param array $pages objects of wikilv pages in subwikilv
     * @param int $swid id of subwikilv
    ...
    protected function add_page_delete_options($pages, $swid, &$table) {
        ...
        foreach ($pages as $page) {
            ...
            $urledit = new moodle_url('/mod/wikilv/edit.php', array('pageid' => $page->id, 'sesskey' => sesskey()));
            $urldelete = new moodle_url('/mod/wikilv/admin.php', array(
                                                                   'pageid'  => $this->page->id,
                                                                   'delete'  => $page->id,
                                                                   'option'  => $this->view,
    ...
    private function print_delete_version() {
        ...
        $versioncount = wikilv_count_wikilv_page_versions($pageid) - 1;
        $versions = wikilv_get_wikilv_page_versions($pageid, 0, $versioncount);
        ...
        if ($versioncount > 0) {
            ...
            if (count($versions) == 1) {
                ...
                foreach ($versions as $version) {
                    ...
                    $radiotoelement = $this->choose_from_radio(array($version->version  => null), 'toversion', 'M.mod_wikilv.deleteversion()', $versioncount, true);
                    $contents[] = array( $radiofromelement . $radiotoelement, $viewlink, $picturelink, $time, $historydate);
                }

                $table = new html_table();
                $table->head = array(get_string('deleteversions', 'wikilv'), get_string('version'), get_string('user'), get_string('modified'), '');
                $table->data = $contents;
                $table->attributes['class'] = 'generaltable mdl-align';
                $table->rowclasses = $rowclass;

                ///Print the form
                echo html_writer::start_tag('form', array('action'=>new moodle_url('/mod/wikilv/admin.php'), 'method' => 'post'));
                echo html_writer::tag('div', html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'pageid', 'value' => $pageid)));
                echo html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'option', 'value' => $this->view));
                echo html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'sesskey', 'value' =>  sesskey()));
                echo html_writer::table($table);
                echo html_writer::start_tag('div', array('class' => 'mdl-align'));
                echo html_writer::empty_tag('input', array('type' => 'submit', 'class' => 'wikilv_form-button', 'value' => get_string('deleteversions', 'wikilv')));
                echo html_writer::end_tag('div');
                echo html_writer::end_tag('form');
            }
        } else {
