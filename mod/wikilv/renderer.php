...
class mod_wikilv_renderer extends plugin_renderer_base {
    public function page_index() {
        global $CFG;
        $output = '';
        // Checking wikilv instance
        if (!$wikilv = wikilv_get_wikilv($this->page->cm->instance)) {
            return false;
        }
        ...
        $gid = !empty($gid) ? $gid : 0;
        if (!$subwikilv = wikilv_get_subwikilv_by_group($this->page->cm->instance, $gid)) {
            return false;
        }
        $swid = $subwikilv->id;
        $pages = wikilv_get_page_list($swid);
        $selectoptions = array();
        foreach ($pages as $page) {
            $selectoptions[$page->id] = format_string($page->title, true, array('context' => $this->page->context));
        }
        $label = get_string('pageindex', 'wikilv') . ': ';
        $select = new single_select(new moodle_url('/mod/wikilv/view.php'), 'pageid', $selectoptions);
        $select->label = $label;
        return $this->output->container($this->output->render($select), 'wikilv_index');
    }
    ...
    public function search_result($records, $subwikilv) {
        ...
        foreach ($records as $page) {
            ...
            $table->width = '100%';
            $table->data = array(array(file_rewrite_pluginfile_urls(format_text($page->cachedcontent, FORMAT_HTML), 'pluginfile.php', $context->id, 'mod_wikilv', 'attachments', $subwikilv->id)));
            $table->colclasses = array('wikilvsearchresults');
    ...
    public function diff($pageid, $old, $new, $options = array()) {
        ...
        $olduser = $old->user;
        $versionlink = new moodle_url('/mod/wikilv/viewversion.php', array('pageid' => $pageid, 'versionid' => $old->id));
        $restorelink = new moodle_url('/mod/wikilv/restoreversion.php', array('pageid' => $pageid, 'versionid' => $old->id));
        $userlink = new moodle_url('/user/view.php', array('id' => $olduser->id));
        // view version link
        $oldversionview = ' ';
        $oldversionview .= html_writer::link($versionlink->out(false), get_string('view', 'wikilv'), array('class' => 'wikilv_diffview'));
        $oldversionview .= ' ';
        // restore version link
        $oldversionview .= html_writer::link($restorelink->out(false), get_string('restore', 'wikilv'), array('class' => 'wikilv_diffview'));

        // userinfo container
        $oldheading = $this->output->container_start('wikilv_diffuserleft');
        // username
        ...
        $oldheading .= $this->output->container_start('wikilv_diffversion');
        $oldheading .= get_string('version') . ' ' . $old->version . $oldversionview;
        $oldheading .= $this->output->container_end();
        // userdate container
        $oldheading .= $this->output->container_start('wikilv_difftime');
        $oldheading .= userdate($old->timecreated, $strdatetime);
        $oldheading .= $this->output->container_end();

        $newuser = $new->user;
        $versionlink = new moodle_url('/mod/wikilv/viewversion.php', array('pageid' => $pageid, 'versionid' => $new->id));
        $restorelink = new moodle_url('/mod/wikilv/restoreversion.php', array('pageid' => $pageid, 'versionid' => $new->id));
        $userlink = new moodle_url('/user/view.php', array('id' => $newuser->id));

        $newversionview = ' ';
        $newversionview .= html_writer::link($versionlink->out(false), get_string('view', 'wikilv'), array('class' => 'wikilv_diffview'));
        // new user info
        $newheading = $this->output->container_start('wikilv_diffuserright');
        ...
        // version
        $newheading .= $this->output->container_start('wikilv_diffversion');
        $newheading .= get_string('version') . '&nbsp;' . $new->version . $newversionview;
        $newheading .= $this->output->container_end();
        // userdate
        $newheading .= $this->output->container_start('wikilv_difftime');
        $newheading .= userdate($new->timecreated, $strdatetime);
        $newheading .= $this->output->container_end();

        $oldheading = html_writer::tag('div', $oldheading, array('class'=>'wikilv-diff-heading header clearfix'));
        $newheading = html_writer::tag('div', $newheading, array('class'=>'wikilv-diff-heading header clearfix'));

        $output  = '';
        $output .= html_writer::start_tag('div', array('class'=>'wikilv-diff-container clearfix'));
        $output .= html_writer::tag('div', $oldheading.$diff1, array('class'=>'wikilv-diff-leftside'));
        $output .= html_writer::tag('div', $newheading.$diff2, array('class'=>'wikilv-diff-rightside'));
        $output .= html_writer::end_tag('div');

        if (!empty($total)) {
            $output .= '<div class="wikilv_diff_paging">';
            $output .= $this->output->container($this->diff_paging_bar(1, $new->version - 1, $old->version, $CFG->wwwroot . '/mod/wikilv/diff.php?pageid=' . $pageid . '&amp;comparewith=' . $new->version . '&amp;', 'compare', false, true), 'wikilv_diff_oldpaging');
            $output .= $this->output->container($this->diff_paging_bar($old->version + 1, $total, $new->version, $CFG->wwwroot . '/mod/wikilv/diff.php?pageid=' . $pageid . '&amp;compare=' . $old->version . '&amp;', 'comparewith', false, true), 'wikilv_diff_newpaging');
            $output .= '</div>';
        }
    ...
    public function diff_paging_bar($minpage, $maxpage, $page, $baseurl, $pagevar = 'page', $nocurr = false) {
        ...
        if ($totalcount > 0) {
            ...
            $output .= get_string('version', 'wikilv') . ':';
            if ($page - $minpage > 0) {
                $pagenum = $page - 1;
    ...
    public function wikilv_info() {
        global $PAGE;
        return $this->output->box(format_module_intro('wikilv', $this->page->activityrecord, $PAGE->cm->id), 'generalbox', 'intro');
    }
    ...
    public function tabs($page, $tabitems, $options) {
        ...
        foreach ($tabitems as $tab) {
            if ($tab == 'edit' && !has_capability('mod/wikilv:editpage', $context)) {
                continue;
            }
            if ($tab == 'comments' && !has_capability('mod/wikilv:viewcomment', $context)) {
                continue;
            }
            if ($tab == 'files' && !has_capability('mod/wikilv:viewpage', $context)) {
                continue;
            }
            if (($tab == 'view' || $tab == 'map' || $tab == 'history') && !has_capability('mod/wikilv:viewpage', $context)) {
                continue;
            }
            if ($tab == 'admin' && !has_capability('mod/wikilv:managewikilv', $context)) {
                continue;
            }
            $link = new moodle_url('/mod/wikilv/'. $tab. '.php', array('pageid' => $pageid));
            if ($linked == $tab) {
                $tabs[] = new tabobject($tab, $link, get_string($tab, 'wikilv'), '', true);
            } else {
                $tabs[] = new tabobject($tab, $link, get_string($tab, 'wikilv'));
            }
        }
    ...
    public function wikilv_print_subwikilv_selector($wikilv, $subwikilv, $page, $pagetype = 'view') {
        ...
        case SEPARATEGROUPS:
            if ($wikilv->wikilvmode == 'collaborative') {
                ...
                //  @TODO: Print here a combo that contains all users of that subwikilv.
                $view = has_capability('mod/wikilv:viewpage', $context);
                $manage = has_capability('mod/wikilv:managewikilv', $context);
                ...
                if ($view && $manage) {
                    ...
                    foreach ($users as $user) {
                        ...
                        if (!empty($groups)) {
                            ...
                            $name = get_string('notingroup', 'wikilv');
                            $options[0][$name]['0' . '-' . $user->id] = fullname($user);
                        }
                    }
                } else {
                    $group = groups_get_group($subwikilv->groupid);
                    if (!$group) {
                        return;
                    }
                    $users = groups_get_members($subwikilv->groupid);
                    foreach ($users as $user) {
                        $options[$group->id][$group->name][$group->id . '-' . $user->id] = fullname($user);
                    }
                }
    ...
    public function wikilv_files_tree($context, $subwikilv) {
        return $this->render(new wikilv_files_tree($context, $subwikilv));
    }
    public function render_wikilv_files_tree(wikilv_files_tree $tree) {
        if (empty($tree->dir['subdirs']) && empty($tree->dir['files'])) {
            $html = $this->output->box(get_string('nofilesavailable', 'repository'));
        } else {
            $htmlid = 'wikilv_files_tree_'.uniqid();
            $module = array('name'=>'mod_wikilv', 'fullpath'=>'/mod/wikilv/module.js');
            $this->page->requires->js_init_call('M.mod_wikilv.init_tree', array(false, $htmlid), false, $module);
            $html = '<div id="'.$htmlid.'">';
    ...
    protected function htmllize_tree($tree, $dir) {
        ...
        /** @lvs recuperação de estudantes do curso em files de wikilv */
        $course = $tree->context->get_course_context()->instanceid;
        $cursolv = new Moodle2CursoLv($course);
        $estudantes = array_keys($cursolv->getEstudantes());
        $gerenciadorNotas = NotasLvFactory::criarGerenciador('moodle2');
        $gerenciadorNotas->setModulo( new Wikilv($tree->subwikilv->wikilvid) );
        // lvs fim
...
class wikilv_files_tree implements renderable {
    public $context;
    public $dir;
    public $subwikilv;
    public function __construct($context, $subwikilv) {
        $fs = get_file_storage();
        $this->context = $context;
        $this->subwikilv = $subwikilv;
        $this->dir = $fs->get_area_tree($context->id, 'mod_wikilv', 'attachments', $subwikilv->id);
    }
}
